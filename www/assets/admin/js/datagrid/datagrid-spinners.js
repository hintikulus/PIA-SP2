...
dataGridRegisterExtension('ublaboo-spinners', {
    before: function (xhr, settings) {
        ...
        if (settings.nette) {
            ...
            spinner_template = $('<div class="overlay ublaboo-spinner"><div class="spinner-border" role="status"><span class="sr-only">Loading...</span></div></div>');

            if (el.is('.datagrid [name="group_action[submit]"]')) {
                //return el.after(spinner_template);
                return el.closest('.card-body').after(spinner_template);
